...
Read all task files in `${CCPM_EPICS_DIR}/$ARGUMENTS/`:
...
```bash
# Check for analysis
if ! test -f ${CCPM_EPICS_DIR}/$ARGUMENTS/{issue}-analysis.md; then
...
Task:
  ...
  prompt: |
    ...
    - ${CCPM_EPICS_DIR}/$ARGUMENTS/{task_file}
    - ${CCPM_EPICS_DIR}/$ARGUMENTS/{issue}-analysis.md
    ...
    Update progress in:
    ${CCPM_EPICS_DIR}/$ARGUMENTS/updates/{issue}/stream-{X}.md
```

### 5. Track Active Agents

Create/update `${CCPM_EPICS_DIR}/$ARGUMENTS/execution-status.md`:
