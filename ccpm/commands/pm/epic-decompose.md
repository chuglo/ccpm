...
1. **Verify epic exists:**
   - Check if `${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md` exists
   - If not found, tell user: "❌ Epic not found: $ARGUMENTS. First create it with: /pm:prd-parse $ARGUMENTS"
   - Stop execution if epic doesn't exist

2. **Check for existing tasks:**
   - Check if any numbered task files (001.md, 002.md, etc.) already exist in `${CCPM_EPICS_DIR}/$ARGUMENTS/`
...
3. **Validate epic frontmatter:**
   ...
   - If invalid, tell user: "❌ Invalid epic frontmatter. Please check: ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md"
...
### 1. Read the Epic
- Load the epic from `${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md`
...
Task:
  ...
  prompt: |
    ...
    For each task:
    1. Create file: ${CCPM_EPICS_DIR}/$ARGUMENTS/{number}.md
...
Save tasks as: `${CCPM_EPICS_DIR}/$ARGUMENTS/{task_number}.md`
