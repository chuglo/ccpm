...
2. **Verify PRD exists:**
   - Check if `${CCPM_PRDS_DIR}/$ARGUMENTS.md` exists
...
3. **Validate PRD frontmatter:**
   ...
   - If frontmatter is invalid or missing, tell user: "❌ Invalid PRD frontmatter. Please check: ${CCPM_PRDS_DIR}/$ARGUMENTS.md"
   - Show what's missing or invalid

4. **Check for existing epic:**
   - Check if `${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md` already exists
...
5. **Verify directory permissions:**
   - Ensure `${CCPM_EPICS_DIR}/` directory exists or can be created
...
### 1. Read the PRD
- Load the PRD from `${CCPM_PRDS_DIR}/$ARGUMENTS.md`
...
Create the epic file at: `${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md` with this exact structure:
...
progress: 0%
prd: ${CCPM_PRDS_DIR}/$ARGUMENTS.md
...
- `${CCPM_EPICS_DIR}/$ARGUMENTS/` (directory)
- `${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md` (epic file)
...
1. Confirm: "✅ Epic created: ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md"
2. Show summary of:
