...
```bash
# Verify epic exists
test -f ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md || echo "❌ Epic not found. Run: /pm:prd-parse $ARGUMENTS"

# Count task files
ls ${CCPM_EPICS_DIR}/$ARGUMENTS/*.md 2>/dev/null | grep -v epic.md | wc -l
```
...
sed '1,/^---$/d; 1,/^---$/d' ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md > /tmp/epic-body-raw.md
...
```bash
task_count=$(ls ${CCPM_EPICS_DIR}/$ARGUMENTS/[0-9][0-9][0-9].md 2>/dev/null | wc -l)
```
...
if [ "$task_count" -lt 5 ]; then
  ...
  for task_file in ${CCPM_EPICS_DIR}/$ARGUMENTS/[0-9][0-9][0-9].md; do
...
sed -i.bak "/^github:/c\github: $epic_url" ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md
sed -i.bak "/^updated:/c\updated: $current_date" ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md
rm ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md.bak
```
...
for task_file in ${CCPM_EPICS_DIR}/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue
...
total_count=$(ls ${CCPM_EPICS_DIR}/$ARGUMENTS/[0-9]*.md 2>/dev/null | wc -l)
parallel_count=$(grep -l '^parallel: true' ${CCPM_EPICS_DIR}/$ARGUMENTS/[0-9]*.md 2>/dev/null | wc -l)
...
cp ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md.backup
...
' ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md.backup > ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md

# Clean up
rm ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md.backup
rm /tmp/tasks-section.md
```

### 6. Create Mapping File

Create `${CCPM_EPICS_DIR}/$ARGUMENTS/github-mapping.md`:
```bash
# Create mapping file
cat > ${CCPM_EPICS_DIR}/$ARGUMENTS/github-mapping.md << EOF
...
for task_file in ${CCPM_EPICS_DIR}/$ARGUMENTS/[0-9]*.md; do
  ...
  echo "- #${issue_num}: ${task_name} - https://github.com/${repo}/issues/${issue_num}" >> ${CCPM_EPICS_DIR}/$ARGUMENTS/github-mapping.md
done

# Add sync timestamp
echo "" >> ${CCPM_EPICS_DIR}/$ARGUMENTS/github-mapping.md
echo "Synced: $(date -u +"%Y-%m-%dT%H:%M:%SZ")" >> ${CCPM_EPICS_DIR}/$ARGUMENTS/github-mapping.md
```
