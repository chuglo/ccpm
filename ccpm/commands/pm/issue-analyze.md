...
1. **Find local task file:**
   - First check if `${CCPM_EPICS_DIR}/*/$ARGUMENTS.md` exists (new naming convention)
   - If not found, search for file containing `github:.*issues/$ARGUMENTS` in frontmatter (old naming)
   - If not found: "❌ No local task for issue #$ARGUMENTS. Run: /pm:import first"

2. **Check for existing analysis:**
   ```bash
   test -f ${CCPM_EPICS_DIR}/*/$ARGUMENTS-analysis.md && echo "⚠️ Analysis already exists. Overwrite? (yes/no)"
   ```
...
Create `${CCPM_EPICS_DIR}/{epic_name}/$ARGUMENTS-analysis.md`:
