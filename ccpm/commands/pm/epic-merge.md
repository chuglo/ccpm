...
2. **Check for active agents:**
   Read `${CCPM_EPICS_DIR}/$ARGUMENTS/execution-status.md`
...
Update `${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md`:
...
feature_list=""
if [ -d "${CCPM_EPICS_DIR}/$ARGUMENTS" ]; then
  cd ${CCPM_EPICS_DIR}/$ARGUMENTS
...
epic_github_line=$(grep 'github:' ${CCPM_EPICS_DIR}/$ARGUMENTS/epic.md 2>/dev/null || true)
...
mkdir -p ${CCPM_EPICS_DIR}/archived/
mv ${CCPM_EPICS_DIR}/$ARGUMENTS ${CCPM_EPICS_DIR}/archived/
echo "✅ Epic archived: ${CCPM_EPICS_DIR}/archived/$ARGUMENTS"
```
...
# Extract epic issue number
epic_github_line=$(grep 'github:' ${CCPM_EPICS_DIR}/archived/$ARGUMENTS/epic.md 2>/dev/null || true)
if [ -n "$epic_github_line" ]; then
  epic_issue=$(echo "$epic_github_line" | grep -oE '[0-9]+$' || true)
else
  epic_issue=""
...
# Close task issues
for task_file in ${CCPM_EPICS_DIR}/archived/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue
