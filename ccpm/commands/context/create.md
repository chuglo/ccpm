...
This command creates the initial project context documentation in `${CCPM_CONTEXT_DIR}/` by analyzing the current project state and establishing comprehensive baseline documentation.
...
- Run: `ls -la ${CCPM_CONTEXT_DIR}/ 2>/dev/null`
- If directory exists and has files:
  - Count existing files: `ls -1 ${CCPM_CONTEXT_DIR}/*.md 2>/dev/null | wc -l`
...
- If `${CCPM_DIR}/` doesn't exist, create it: `mkdir -p ${CCPM_CONTEXT_DIR}/`
- Verify write permissions: `touch ${CCPM_CONTEXT_DIR}/.test && rm ${CCPM_CONTEXT_DIR}/.test`
...
**Common Issues:**
- **No write permissions:** "❌ Cannot write to ${CCPM_CONTEXT_DIR}/. Check permissions."
...
📁 Created context in: ${CCPM_CONTEXT_DIR}/
...
- Target directory: `${CCPM_CONTEXT_DIR}/` (create if needed)
