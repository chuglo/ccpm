...
if [ -d "${CCPM_PRDS_DIR}" ]; then
  echo "📄 PRDs:"
  results=$(grep -l -i "$query" ${CCPM_PRDS_DIR}/*.md 2>/dev/null)
  if [ -n "$results" ]; then
...
if [ -d "${CCPM_EPICS_DIR}" ]; then
  echo "📚 Epics:"
  results=$(find ${CCPM_EPICS_DIR} -name "epic.md" -exec grep -l -i "$query" {} \; 2>/dev/null)
  if [ -n "$results" ]; then
    for file in $results; do
...
if [ -d "${CCPM_EPICS_DIR}" ]; then
  echo "📝 Tasks:"
  results=$(find ${CCPM_EPICS_DIR} -name "[0-9]*.md" -exec grep -l -i "$query" {} \; 2>/dev/null | head -10)
  if [ -n "$results" ]; then
    for file in $results; do
      epic_name=$(basename $(dirname "$file"))
