...
for updates_dir in ${CCPM_EPICS_DIR}/*/updates/*/; do
...
echo ""
echo "📊 Quick Stats:"
total_tasks=$(find ${CCPM_EPICS_DIR} -name "[0-9]*.md" 2>/dev/null | wc -l)
open_tasks=$(find ${CCPM_EPICS_DIR} -name "[0-9]*.md" -exec grep -l "^status: *open" {} \; 2>/dev/null | wc -l)
closed_tasks=$(find ${CCPM_EPICS_DIR} -name "[0-9]*.md" -exec grep -l "^status: *closed" {} \; 2>/dev/null | wc -l)
